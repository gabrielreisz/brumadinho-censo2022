...
def _titulo(fig, titulo: str, subtitulo: str | None = None) -> float:
    ...
    fig.suptitle(
        titulo, fontsize=14, fontweight="bold", color=COR_TEXTO_PRIMARIO,
        x=0.01, ha="left", y=y_titulo, va="top",
    )
    if subtitulo:
        y_subtitulo = 1 - (0.42 / fig_h)
        fig.text(0.01, y_subtitulo, subtitulo, fontsize=10, color=COR_TEXTO_SECUNDARIO, ha="left", va="top")
        return 1 - (0.78 / fig_h)
    return 1 - (0.40 / fig_h)
...
def _salvar(fig, nome_arquivo: str) -> None:
    ...
    destino = DIR_FIGURAS / nome_arquivo
